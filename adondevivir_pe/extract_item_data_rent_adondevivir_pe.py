...
for i in range(0, 39):
    ...
    with open("./items_rent_html/adondevivir-rent-item-{}.txt".format(i), "r") as item:
        content = item.read()
        ...
        try:
            ...
            with open("./logs/scans/adondevivir-rent-items-{}.txt".format(date.today()), "a") as writeItem:
                writeItem.write('"{}", \n'.format(param_value_getter))
                writeItem.close()

        except AttributeError:
            with open(
                "./logs/errored/adondevivir-rent-errored-{}.txt".format(date.today()),
                "a",
            ) as errored:
                errored.write('"{}", \n'.format(i))

...
for i in range(0, 39):
    ...
    with open("./items_buy_html/adondevivir-buy-item-{}.txt".format(i), "r") as item:
        content = item.read()
        ...
        try:
            ...
            param_value_getter = {
                ...
                "extras": [{"pabuy": "Comodidades", "fields": [{"name": "Piscina"}]}],
            }
            laraIndex(param_value_getter)
            with open("./logs/scans/adondevivir-buy-items-{}.txt".format(date.today()), "a") as writeItem:
                writeItem.write('"{}", \n'.format(param_value_getter))
                writeItem.close()
            

        except AttributeError:
            with open("./logs/errored/adondevivir-buy-errored-{}.txt".format(date.today()), "a") as errored:
                errored.write('"{}", \n'.format(i))
